...
export class Parasite extends Group {
    ...
    constructor(partCount: number, tubeMinMax: Vector2, tubePeriods: number, ovaShellMinMax: Vector2, skin: string) {
        ...
        for (let i = 0; i < this.partCount; i++) {
            ...
            this.pts[i] = pos;
            let spd = new Vector3(randFloat(-14.8, 14.8), randFloat(-14.8, 14.8), randFloat(-14.8, 14.8));
        ...
        let isWireFrame = (Math.round(Math.random() * 40) % 4 == 0) ? true : false;
        //isWireFrame = true;
        let protoMat = new MeshPhongMaterial({
            color: colTube,
            specular: 0xDDDDDD + Math.random() * 0x444444,
            reflectivity: .8,
        ...
        this.tubeMaxAmp = randFloat(20, 125);
    ...
    move(time: number) {
        ...
        for (let i = 0; i < this.partCount; i++) {
            ...
            this.parts[i].move();
            if (this.parts[i].pos.length() >= this.ovaShellRadMin * .35 + sin(time * PI / .1) * 1.2) {
                this.parts[i].pos.normalize();
                this.parts[i].pos.multiplyScalar(this.ovaShellRadMin * .35 + sin(time * PI / .1) * 1.2);
