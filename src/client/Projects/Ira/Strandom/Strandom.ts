...
import { BoxGeometry, BufferGeometry, Group, InstancedMesh, Line, LineBasicMaterial, Matrix4, MeshBasicMaterial, SphereGeometry, Vector3, Vector2, CatmullRomCurve3 } from "three";
import { randFloat, randInt } from "three/src/math/MathUtils";
import { cos, GeometryDetail, PI, sin } from "../../../PByte3/IJGUtils";

export class Strandom extends Group {
                ...
                if (i !== j) {
                    if (this.pos[i].distanceTo(this.pos[j]) < this.dynamicLineLenCount) {
                        if (this.lineCounter < this.lineCount) {
                            ...
                            tempPts.push(new Vector3(this.pos[i].x, this.pos[i].y, this.pos[i].z))
                            for (let k = 1; k < this.curvePointCount - 1; k++) {
                                tempPts.push(new Vector3(randFloat(this.pos[k].x * .4, this.pos[k].x * 1.4),
                                    randFloat(this.pos[k].y * .4, this.pos[k].y * 1.4),
                                    randFloat(this.pos[k].z * .4, this.pos[k].z * 1.4)))
                            }
                            tempPts.push(new Vector3(this.pos[j].x, this.pos[j].y, this.pos[j].z))

                            const curve = new CatmullRomCurve3(tempPts);
                            const points = curve.getPoints(this.curvePointCount - 2);
                            // console.log(tempPts.length);

                            let p = this.lineMesh[this.lineCounter].geometry.attributes.position;
                            p.needsUpdate = true;

                            // p.setXYZ(0, points[0].x, points[0].y, points[0].z);
                            for (let k = 0; k < this.curvePointCount - 1; k++) {
                                p.setXYZ(k, points[k].x, points[k].y, points[k].z);
                            }
                            // p.setXYZ(this.curvePointCount - 1, points[points.length - 1].x, points[points.length - 1].y, points[points.length - 1].z);
