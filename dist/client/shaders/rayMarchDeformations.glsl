...
mat2 rot(float a) { // returns 2d rotation matrix
  float s = sin(a);
  float c = cos(a);
  return mat2(c, -s, s, c);
}
...
//  float boxDist = sdBox(p-vec3(-3.0,0.75,6.0), vec3(0.75));
  // distance to torus
  float torDist = sdTorus(p-vec3(0.0, 1.5, 6.0), vec2(1.5, 0.3));

// order matters - trn rot  vs  rot trn
 //vec3 boxPosit = p;
 //boxPosit -= vec3(-1.0,1.75,7.0);  // translation
 //boxPosit.xy *= rot(u_time);  // rotation - missing axis
 // float boxDist = sdBox(boxPosit, vec3(0.75, 0.75, 0.75));
  // distance to capped cylinder
//  float cylDist = sdCyl(p, vec3(0.0, 0.3, 4.0), vec3(3.0, 0.3, 5.0), 0.3);

  float d = torDist;
//  d = min(d, sphereDist);
  d = smin(d, sphereDist, 20.0);
//  d = min(d, capsDist);
  d = min(d, planeDist);
//  d = min(d, boxDist);
//  d = min(d, cylDist);
...
float getLight(vec3 p) {
  vec3 lightPos = vec3(-2.0, 10.0, 4.0);
  lightPos.xz += vec2(sin(u_time), cos(u_time)) * 4.5;
