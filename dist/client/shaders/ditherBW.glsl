...
uniform sampler2D u_tex_0; // ../1.jpg
uniform sampler2D u_tex_1; // ../IMG_2572.png
uniform sampler2D u_tex_2; // ../street.jpg

#define thresh 0.2871
...
void main(void) {
  ...
  vec3 col = texture2D(u_tex_0, uv).rgb;// vec3(0.0);
  // n += col;
  // n /= 9.0;
  vec3 avgd = vec3(step(noiseAndFBM(vec3(uv.x, uv.y, 1.0)*1000.0), col));
  vec3 finalCol = mix(col, avgd, (sin(u_time)*0.5+0.5)); 
