...
//  vec2 st = vUv * 2.0 - 1.0;
  ...
  float r = snoise(vec3(st,u_time/10.));//u_time/10.0));
...
//  vec2 st = vUv * 2.0 - 1.0;
  ...
  float r = snoise(vec3(st,1.0))*sin(u_time)*1.5;//u_time/10.0));
  float g = snoise(vec3(st,1.0))*cos(u_time)*4.0;// u_time/5.0));
  float b = snoise(vec3(st,1.0))*sin(u_time)*2.0;// u_time/7.0));
  return vec4(r, g, b, 1.0);
//  return vec4(snoise(vec3(st.x, st.y, u_time)));
}
