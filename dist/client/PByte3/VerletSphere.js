...
import { Color, Group, Vector3 } from '/build/three.module.js';
...
export class VerletSphere extends Group {
    constructor(pos, radii, spineCount, sliceCount) {
        super();
        this.nodesOrig = []; // for convenience
        this.nodes = []; // for convenience
        this.nodesCore = []; // for convenience
        this.nodes2D = [[]];
        this.nodes2D_orig = [[]];
        this.sticks = [];
        this.tendrils = [];
        ...
        for (var i = 1; i < sliceCount; i++) { // calc edge nodes
            ...
            for (var j = 0; j < spineCount; j++) { // rotate and collect edge nodes
                k = spineCount * i + j;
                let v = new Vector3(z * Math.sin(phi) + x * Math.cos(phi), y, z * Math.cos(phi) - x * Math.sin(phi));
                let vCore = new Vector3(zCore * Math.sin(phi) + xCore * Math.cos(phi), yCore, zCore * Math.cos(phi) - xCore * Math.sin(phi));
                let v2 = new Vector3(z * Math.sin(phi) + x * Math.cos(phi), y, z * Math.cos(phi) - x * Math.sin(phi));
                let v2Core = new Vector3(zCore * Math.sin(phi) + xCore * Math.cos(phi), yCore, zCore * Math.cos(phi) - xCore * Math.sin(phi));
    ...
    setNodeVisibility(isNodeVisible) {
    }
    setNodeColor(color, alpha) {
    }
    setStickColor(color, alpha) {
    ...
    pulseIndices(indices, amps, freqs, thetas) {
        for (var i = 0; i < indices.length; i++) {
